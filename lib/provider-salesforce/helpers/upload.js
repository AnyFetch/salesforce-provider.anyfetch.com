...
var uploadDocument = function(doc, anyfetchClient, datas, cb) {
  // console.log(doc, '\n');
...
var uploadFile = function(doc, fileConfig, anyfetchClient, datas, cb) {
  //Stream the file
...
var deleteDocument = function(doc, anyfetchClient, datas, cb) {
  // Identifier creation
...
module.exports = function(doc, anyfetchClient, datas, cb) {
  if (doc.IsDeleted) {
    deleteDocument(doc, anyfetchClient, datas, cb);
  } else {
    ...
    formatters[type](doc, datas, function(err, formatedDoc, fileConfig) {
      ...
      if (fileConfig) {
        uploadFile(formatedDoc, fileConfig, anyfetchClient, datas, cb);
      } else {
        uploadDocument(formatedDoc, anyfetchClient, datas, cb);
      }
    });
  }
};
