...
var getColumns = function(oauth, objectType, cb) {
  ...
  org.getDescribe({type: objectType, oauth: oauth}, function(err, res) {
    if (err) {
      return cb(err);
    }
...
var retrieveFromType = function(oauth, type, cb) {
  getColumns(oauth, type, function(err, columns) {
    ...
    org.queryAll({ query: q, oauth: oauth}, function(err, res) {
      if (err) {
        return cb(err);
      }
      cb(null, res.records);
...
var mapReleadElementObjects = function(contactId, contactName, releatedRecord, cb) {
  // Formattable Types
  ...
  releatedObjectsTypes.forEach(function(type) {
    ...
    if (releatedRecord[type] !== null) {
      releatedRecord[type].records.forEach(function(realeatedObject) {
        ...
        realeatedObject.ReleatedParentName = contactName;
...
var getReleatedElement = function(oauth, cb) {
  var q = 'SELECT Id, Name,  (SELECT Id, Subject, Description, StartDateTime, EndDateTime, CreatedDate, IsDeleted FROM Events), ';
    q += '(SELECT Id, Title, Body, CreatedDate, IsDeleted FROM Notes), ';
    q += '(SELECT Id, BodyLength, Name, Description, CreatedDate, IsDeleted FROM Attachments), ';
    q += '(SELECT Id, Subject, Status, ActivityDate, Description, CreatedDate, IsDeleted FROM Tasks) ';
    q += 'FROM Contact';
  ...
  org.queryAll({query: q, oauth: oauth}, function(err, res) {
    ...
    res.records.forEach( function(record) {
      ...
      var contactName = record.Name;
      mapReleadElementObjects(contactId, contactName, record, function(err, res) {
...
var retrieveAttachements = function(oauth, cb) {
  var q = 'Select Id, Name, BodyLength, CreatedDate, IsDeleted From Attachment';
