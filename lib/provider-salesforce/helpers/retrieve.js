...
var getReleatedElement = function(oauth, cb) {
  var q = 'SELECT Id,  (SELECT Id, Subject, Description, StartDateTime, EndDateTime, CreatedDate FROM Events), ';
    q += '(SELECT Id, Title, Body, CreatedDate FROM Notes), ';
    q += '(SELECT Id, BodyLength, Name, Description, CreatedDate FROM Attachments), ';
    q += '(SELECT Id, Subject, Status, ActivityDate, Description, CreatedDate FROM Tasks) ';
    q += 'FROM Contact';
